...
export const QueriesEditor: React.FC<StandardEditorProps<QueryConfig[]>> = ({ value = [], onChange, context }) => {
  ...
  const update = (next: QueryConfig) => onChange(value.map((q, idx) => (idx === index ? next : q)));

  const query: QueryConfig | null = index !== null ? value[index] : null;

  const selectOptions = value
    .map((q, idx) => ({
      label: `${q.refId || '(empty)'} \u2013 ${TYPE_LABELS[q.type] ?? q.type} (#${q.id})`,
      value: idx,
    }))
